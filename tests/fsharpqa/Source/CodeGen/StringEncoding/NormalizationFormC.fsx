...
let content =
    ...
    [
        for a in [0 .. 0x123] do
            yield sprintf "        yield return (int)(\"\\u%04x\".Normalize(System.Text.NormalizationForm.FormC)[0]);" a
    ] @
    [
        sprintf "    }"
        ...
        sprintf "       {"
        sprintf "           sb.AppendLine(System.String.Format(\"if {0} <> int (\\\"\\\\u{1:X4}\\\".Normalize(System.Text.NormalizationForm.FormC).[0]) then printfn \\\"Difference detected at u{1:X4}\\\"; exit 1\", v, i++));"
        sprintf "       }"
